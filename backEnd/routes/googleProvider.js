...
router.patch('/register', tokenVerifier, async(req, res) => {
    try {
        ...
        const updated = await User.updateOne({username: result.username},
            {$set: {googleProvider: {authToken: req.body.token,
                    active: true}}});
        res.json({success:"true", user: result});
        }catch(err){res.json({success: 'false', message: "couldn't get service's token"})}
});
...
router.patch('/logout', tokenVerifier, async (req, res) => {
    try {
        const result = await jwtToUser(req.header('access-token'))
        const updated = await User.updateOne({username: result.username},
            {$set: {googleProvider: {authToken: "",
                    active: false}}});
        res.json({success: 'true', message: 'logged out successfully of Google'});
