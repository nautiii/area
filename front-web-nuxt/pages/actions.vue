<template>
  <v-container v-if="$store.state.user.isLogin">
    <v-row class="pa-4" width="100%" justify="center">
      <v-dialog
      ...
      >
        <template v-slot:activator="{ on, attrs }">
          <v-btn dark v-bind="attrs" v-on="on">Add a new action</v-btn>
        </template>
        <template>
          <v-card>
    ...
    <v-row class="actions-row pa-3 fill-height">
      <Action
        class="pa-3"
        v-for="(item, index) in this.$store.state.user.actions"
        justify="space-between"
        :key="index"
        :actionServiceProps="item.action.service"
        :actionProps="item.action.name"
        :reactionServiceProps="item.reaction.service"
        :reactionProps="item.reaction.name"
        :id="item._id"
      />
    </v-row>
  </v-container>
...
export default {
  ...
  methods: {
    ...
    async addNewAction() {
      ...
      let actionID = await this.setNewAction(newAction)
      newAction._id = actionID

      this.$store.commit('user/addAction', newAction)

      this.modalVisible = false
    },

    async setNewAction(newAction) {
      let responseData = {}
    ...
    async fetchUserActions() {
      ...
      const config = {
        method: 'get',
        url: 'http://localhost:8080/api/user/actions',
        headers: {
          'Content-Type': 'application/json',
          'access-token': this.$store.state.user.accessToken
        }
      ...
      if (responseData.success) {
        this.$store.commit('user/resetActions')
        responseData.UserAction.forEach((element) => {
          this.$store.commit('user/addAction', element)
        })
      }
    },
